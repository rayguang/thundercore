...
def  func_encrypt(message):
    ...
    try:
        ...
        sql_select_query = """select user_id, exchange_name, exchange_id, apikey, apisecret, exchange_uid from exchange_api_staging where user_id = %s and exchange_name = %s """
    ...
    try:
        ...
        sql_insert_query            = """ INSERT INTO `exchange_API` (`user_id`, `exchange_name`, `exchange_id`, `apikey`, `apisecret`, `exchange_uid`) VALUES (%s, %s, %s, %s, %s, %s)"""
        sql_insert_staging_query    = """ INSERT INTO `exchange_API_staging` (`user_id`, `exchange_name`, `exchange_id`, `apikey`, `apisecret`, `exchange_uid`) VALUES (%s, %s, %s, %s, %s, %s)"""
        delete_tuple            = (userId, exchange)
        insert_tuple            = (userId, exchange, _exchange_id, _key, secret_encrypted, _exchange_uid)
        insert_staging_tuple    = (userId, exchange, _exchange_id, _key, _secret, _exchange_uid)
...
print ("*******************************************************")
print ("Connecting to pusher APP ID {}".format(os.getenv("APP_ID_TRADING_PRD")))
logging.info ("Connecting to pusher APP ID {}".format(os.getenv("APP_ID_TRADING_PRD")))
