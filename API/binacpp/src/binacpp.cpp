...
//------------
void
BinaCPP::get_tradingPairs( Json::Value &json_result)
{
    BinaCPP_logger::write_log( "<BinaCPP::get_tradingPairs>" ) ;
    ...
    if ( str_result.size() > 0 ) {
        ...
        try {
            ...
            BinaCPP_logger::write_log( "<BinaCPP::get_tradingPairs> Error ! %s", e.what() );
        }
        BinaCPP_logger::write_log( "<BinaCPP::get_tradingPairs> Done." ) ;

    } else {
        BinaCPP_logger::write_log( "<BinaCPP::get_tradingPairs> Failed to get anything." ) ;
    }
}
...
// Get Single Pair's Price
void
BinaCPP::get_price( const char *symbol, Json::Value &json_result )
{
	BinaCPP_logger::write_log( "<BinaCPP::get_price>" ) ;
...
// Get Recent Trades
void
BinaCPP::get_recentTrades(
    const char *symbol,
    int limit,
    Json::Value &json_result )
{

    BinaCPP_logger::write_log( "<BinaCPP::get_recentTrades>" ) ;
    ...
    url.append( querystring );
    BinaCPP_logger::write_log( "<BinaCPP::get_recentTrades> url = |%s|" , url.c_str() ) ;
    ...
    if ( str_result.size() > 0 ) {
        ...
        try {
            ...
            BinaCPP_logger::write_log( "<BinaCPP::get_recentTrades> Error ! %s", e.what() );
        }
        BinaCPP_logger::write_log( "<BinaCPP::get_recentTrades> Done." ) ;

    } else {
        BinaCPP_logger::write_log( "<BinaCPP::get_recentTrades> Failed to get anything." ) ;
    }
}
...
// Ray Guang changes: add market order function: fix TimeInForce not required for market
void
BinaCPP::send_order_market(
	const char *symbol,
	const char *side,
	const char *type,
	//const char *timeInForce,
